...
# TETRIS - 80x24 terminal version

set -u 
up and SIGUSR2 to quit
...
update_score() {
    ...
    set_bold
    set_fg $SCORE_COLOR
    xyprint $((SCORE_X - 1)) $((SCORE_Y - 1)) "┌────────────────────┐"
    xyprint $((SCORE_X - 1)) $SCORE_Y         "│ Lines completed: $lines_completed │"
    xyprint $((SCORE_X - 1)) $((SCORE_Y + 1)) "│ Level:           $level │"
    xyprint $((SCORE_X - 1)) $((SCORE_Y + 2)) "│ Score:           $score │"
    xyprint $((SCORE_X - 1)) $((SCORE_Y + 3)) "└────────────────────┘"
    reset_colors
}
